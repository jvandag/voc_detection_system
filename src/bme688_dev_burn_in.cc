...
bme68xData sensorData[N_KIT_SENS];
...
void setup()
{
  ...
  for (uint8_t i = 0; i < N_KIT_SENS; i++) {
    ...
    if (bme[i].checkStatus()) {
      Serial.println(String("Init failed on sensor ") + i + ": " + bme[i].statusString());
      panicLeds();
    }
  }
  ...
  for (uint8_t i = 0; i < N_KIT_SENS; i++) {
    ...
    uint16_t sharedHeatrDur =
      MEAS_DUR - (uint16_t)(bme[i].getMeasDur(BME68X_PARALLEL_MODE) / INT64_C(1000));
...
void loop()
{
  ...
  for (uint8_t i = 0; i < N_KIT_SENS; i++) {
    if (bme[i].fetchData()) {
      ...
      while (bme[i].getData(sensorData[i])) {
        // Optionally, check flags if you want to verify stability
        // bool heat_ok = (sensorData[i].status & BME68X_HEAT_STAB_MSK);
        // bool gas_ok  = (sensorData[i].status & BME68X_GASM_VALID_MSK);
